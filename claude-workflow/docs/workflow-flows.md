...
flowchart TD
    ...
    G --> H{Product → Spec handoff ready?}
...
|---|---|---|
| Read-only / advisory | `/workflow-router`, `/explain-code`, `/review-diff`, `/spec-review`, `/spec-plan-review`, `/spec-task-review`, `/spec-status`, Product Mode commands | No edits unless user explicitly asks to persist/apply something. |
